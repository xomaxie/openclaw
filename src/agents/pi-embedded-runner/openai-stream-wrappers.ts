...
import type { AssistantMessageEvent, SimpleStreamOptions } from "@mariozechner/pi-ai";
...
function hasVisibleAssistantContent(message: { content?: unknown } | undefined): boolean {
...
export function createOpenAINonVisibleStopGuardWrapper(baseStreamFn: StreamFn | undefined): StreamFn {
    ...
    queueMicrotask(async () => {
      try {
        for await (const event of baseStream) {
          if (event.type === "done") {
            const doneEvent = event as Extract<AssistantMessageEvent, { type: "done" }>;
            if (doneEvent.reason === "stop" && !hasVisibleAssistantContent(doneEvent.message)) {
              guarded.push({
                type: "error",
                reason: "error",
                error: {
